    ...
    fun createNewSwabReport(isExist: Boolean, idSwabReport: Int) {
        ...
        Observable.just(db)
            ...
            .subscribe ({
                ...
                /* SERGIO POZOS*/
                val getPozosDao = it.getPozosDao()
                /*  FIN SERGIO */
                ...
                if (!isExist) {
                    ...
                    /* SERGIO ANEXO K - TABLA TIPS ANALISIS DE RIESGO */
                    sharedPreferencesManager.getTipsAnalisisRiesgoList()?.map { fila ->
                        val tipsAnalisisRiesgoLista = TipsAnalisisRiesgoEntity(
                            idtips_analisisderiesgo =  fila.idtips_analisisderiesgo,
                            tips_analisisderiesgo = fila.tips_analisisderiesgo,
                            grupo = fila.grupo,
                            activo = fila.activo
                        )
                        getTipsAnalisisRiesgoDao.insertar(tipsAnalisisRiesgoLista)
                    }
                    /* FIN SERGIO */

                    /* SERGIO ANEXO I - TABLA POZOS */
                    sharedPreferencesManager.getPozosList()?.map { fila ->
                        val pozosLista = PozosEntity(
                            idpozo = fila.idpozo,
                            pozo = fila.pozo,
                            bateria = fila.bateria,
                            diametro = fila.diametro,
                            diametro_fraccion=fila.diametro_fraccion,
                            na_td=fila.na_td
                        )
                        getPozosDao.insertar(pozosLista)
